...
int main(){
  ...
  struct list* linkedlist = (struct list*) malloc(sizeof(struct list));
  struct list_item* listhead = (struct list_item*) malloc(sizeof(struct list_item));
  struct list_item* listtail = (struct list_item*) malloc(sizeof(struct list_item));
  listhead->next = listtail;
  listtail->prev = listhead;
  linkedlist->head = listhead;
  linkedlist->tail = listtail;
  linkedlist->length = 0;
	...
	do {
		...
		switch(option){
			...
			case PRINT_LIST:				// print list
				...
				for (int j = 0; j < linkedlist->length; j = j + 1) {
