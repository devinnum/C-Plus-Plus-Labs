...
void TreeCalc::insert(const string& val) {
  ...
  if (val[0] == '/' || val[0] == '*' || (val[0] == '-' && val.size() == 1)
      || val[0] == '+') {
...
int TreeCalc::calculate(TreeNode* tree) const {
  ...
  if ((isdigit(tree->left->value[0]) || tree->left->value.size() > 1) &&
      (isdigit(tree->right->value[0]) || tree->right->value.size() > 1)) {
    if (tree->value[0] == '+') {
      return stoi(tree->left->value) + stoi(tree->right->value);
    }
  ...
  else if (!(isdigit(tree->left->value[0]) || tree->left->value.size() > 1)
	   && !(isdigit(tree->right->value[0]) || tree->right->value.size()
	   > 1)) {
    if (tree->value[0] == '+') {
    ...
    if (tree->value[0] == '+') {
      return calculate(tree->left) + stoi(tree->right->value);
    }
    else if (tree->value[0] == '-') {
      return calculate(tree->left) - stoi(tree->right->value);
    }
    ...
    if (tree->value[0] == '+') {
      return stoi(tree->left->value) + calculate(tree->right);
    }
    else if (tree->value[0] == '-') {
      return stoi(tree->left->value) - calculate(tree->right);
    }
