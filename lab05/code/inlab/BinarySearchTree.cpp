...
BinaryNode* BinarySearchTree::insert(BinaryNode*& n, const string& x) {
  ...
  if (x < n->value) {
    if (n->left == NULL) {
      n->left = new BinaryNode();
      return n->left;
    }
    else{ 
      return insert(n->left, x);
    }
  }

  if (x > n->value) {
    if (n->right == NULL) {
      n->right = new BinaryNode();
      return n->right;
    }
    
    else{
      return insert(n->right, x);
    }
  }
}
...
string BinarySearchTree::pathTo(const string& x) const {
  ...
  string y = "";
  string& builder = y;
  pathTo(root, x, builder);
  return builder;
    // YOUR IMPLEMENTATION GOES HERE
}

BinaryNode* BinarySearchTree::pathTo(BinaryNode* n, const string& x, string& builder) const {
  ...
  if (x < n->value) {
    builder = builder + n->value + " " ;
    pathTo(n->left, x, builder);
  }

  if (x > n->value) {
    builder = builder +  n->value + " " ;
    pathTo(n->right, x, builder);
  }
}
...
BinaryNode* BinarySearchTree::numNodes(BinaryNode* n, int& count) const {
  count++;

  if (n->left != NULL) {
    numNodes(n->left, count);
  }

  if (n->right != NULL) {
    numNodes(n->right, count);
  }
